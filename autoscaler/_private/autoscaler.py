...
class StandardAutoscaler:
    ...
    def terminate_nodes_to_enforce_config_constraints(self, now: float):
        ...
        for node_id in sorted_node_ids:
            ...
            # execute node termination request
            node_resource_tags = self._node_resources(node_id)
            logger.info("Terminate requests Autoscaler: " + str(self.load_metrics.terminate_requests))
            logger.info("Tags for nodes: " + str(node_resource_tags))
            for resource_tag in self.load_metrics.terminate_requests.keys():
                if resource_tag in node_resource_tags.keys():
                    logger.info(f"Terminated node {node_id} on request of the user")
                    self.schedule_node_termination(node_id, "termination requested by user", logger.info)
        self.load_metrics.set_terminate_requests({})
