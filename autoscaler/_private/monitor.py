...
class Monitor:
    ...
    def update_terminate_requests(self):
        if not _internal_kv_initialized():
            return
        val = _internal_kv_get("terminate_node_by_resource_tag")
        if val:
            try:
                resource_tag = json.loads(val)
                self.load_metrics.set_terminate_requests(resource_tag)
                logger.info("Terminate request received: " + str(resource_tag))
                _internal_kv_del("terminate_node_by_resource_tag")
            except Exception:
                logger.exception("Error parsing terminate resource tags")
    ...
    def _run(self):
        ...
        while True:
            try:
                ...
                self.update_terminate_requests()
