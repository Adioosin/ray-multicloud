...
@DeveloperAPI
def terminate_node_by_resource_tag(resource_tag: dict):
    """Terminates the node based on the resource type"""
    if isinstance(resource_tag, dict):
        for key in resource_tag.keys():
            if not (isinstance(key, str) and isinstance(resource_tag[key], int)):
                raise TypeError(
                    "each resource_tag key should be str and value as int."
                )
    else:
        raise TypeError("each resource_tag should be a Dict.")

    return commands.terminate_node_by_resource_tag(resource_tag)
